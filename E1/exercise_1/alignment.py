...
def procrustes_align(pc_x, pc_y):
    ...
    center_coor_x = np.mean(pc_x, axis=0) #shape: 3 * 1
    centered_pc_x = pc_x - center_coor_x
    center_coor_y = np.mean(pc_y, axis=0)
    centered_pc_y = pc_y - center_coor_y
    X = centered_pc_x.reshape(3, -1)
    Y = centered_pc_y.reshape(3, -1)
    # Y = RX ==> use pseudo inverse
    X_pinv = np.linalg.pinv(X)
    R = np.dot(Y, X_pinv) # 3*3
    t = center_coor_y - np.dot(center_coor_x, R)
